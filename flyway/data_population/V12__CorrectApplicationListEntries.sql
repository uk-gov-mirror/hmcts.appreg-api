-- History
-- Matthew Harman       V1.0    05/11/2025      Initial script to correct ApplicationListEntries data
--
...
DECLARE
        ...
        entry_count INTEGER;
        special_list INTEGER := 11; -- List 11 will have 8 entries
BEGIN
...
VALUES ('12', 'test_support', 'check_data_expected_v12_present')
...
BEGIN
   -- Check that list 11 has exactly 8 entries as expected
   IF NOT EXISTS (SELECT 1 WHERE (SELECT COUNT(*) FROM application_list_entries WHERE al_al_id = 11) = 8) THEN
        RAISE EXCEPTION 'Expected 8 entries for list 11, got %', (SELECT COUNT(*) FROM application_list_entries WHERE al_al_id = 11);
   END IF;
