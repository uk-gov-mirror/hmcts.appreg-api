...
import uk.gov.hmcts.appregister.common.model.Pageable;
...
public class CriminalJusticeAreaController implements CriminalJusticeAreasApi {
    ...
    private final Pageable appRegPageable;
    ...
    @Override
    @PreAuthorize(RoleNames.USER_ROLE_OR_ADMIN_ROLE_RESTRICTION)
    public ResponseEntity<CriminalJusticeAreaPage> getCriminalJusticeAreas(String code, String description,
                                                                           Integer page, Integer size, List<String> sort) {
        // parse the pageable, supplying a default sort order if
        org.springframework.data.domain.Pageable pageable = appRegPageable.from(page, size, sort, CriminalJusticeArea_.CODE, Sort.Direction.ASC);

        // validate the sort parameters
        pageable.getSort().get().forEach(o -> sortValidator.validate(o.getProperty()));

        CriminalJusticeAreaPage criminalJusticeAreaPage = criminalJusticeService.findAll(code, description, pageable);
