...
public interface NationalCourtHouseService {
   ...
   * {@link #search(String, String, LocalDate, LocalDate, LocalDate, LocalDate, Pageable)} to avoid
   * large payloads.
   *
   ...
   *
   * <p>Implementations should translate a missing entity to a {@code 404} (e.g., by throwing {@code
   ...
   *       Implementations commonly treat {@code endDate = NULL} as “ongoing” and include those when
   *       {@code endDateFrom} is supplied.
   * </ul>
   *
   * <p><strong>Paging/Sorting:</strong> Provided via {@link Pageable}. Controllers typically supply
   * a default sort (e.g., {@code Sort.by("name").ascending()}). Results should already be mapped to
   * DTOs.
   *
