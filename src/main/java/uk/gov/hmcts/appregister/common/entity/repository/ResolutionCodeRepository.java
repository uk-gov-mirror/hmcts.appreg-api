...
public interface ResolutionCodeRepository
   ...
   *   <li>{@code title}: partial, case-insensitive match on {@code resolution_code_title} (ILIKE).
   *   <li>{@code startFrom}/{@code startTo}: inclusive bounds on {@code
   *       resolution_code_start_date}.
   *   <li>{@code endFrom}: include records where {@code resolution_code_end_date} is <em>null</em>
   *       (treated as ongoing) <strong>or</strong> {@code >= endFrom}.
   ...
   * Returns an {@link Optional} so that callers must handle the "not found" case explicitly.
   *
