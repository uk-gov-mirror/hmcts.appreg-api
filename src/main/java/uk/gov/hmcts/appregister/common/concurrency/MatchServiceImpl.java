...
public class MatchServiceImpl implements MatchService{
    ...
    public <T> MatchResponse<T> matchOnRequest(UUID id,
        ...
        if (request != null && request.getEtag() != null) {
            ...
            // if there is a clash between the etag in the request and the entity data then throw an exception
            if (!generateEtag.equals(request.getEtag())) {
                throw new AppRegistryException(CommonAppError.MATCH_ETAG_FAILURE, "ETag specified %s does not match identified record".formatted(request.getEtag()));
            }
        }
