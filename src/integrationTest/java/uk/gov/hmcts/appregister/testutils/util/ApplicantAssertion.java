...
public class ApplicantAssertion {
    public static void validatePerson(Person applicant,
        ...
        Assertions.assertEquals(
            applicant.getName().getSurname(),
            standardApplicant.getApplicantSurname()
        );
        Assertions.assertEquals(
            applicant.getName().getThirdForename(),
            standardApplicant.getApplicantForename3()
        );
        Assertions.assertEquals(
            applicant.getName().getFirstForename(),
            standardApplicant.getApplicantForename2()
        );
        Assertions.assertEquals(
            applicant.getName().getSecondForename(),
            standardApplicant.getApplicantForename3()
        );
        Assertions.assertEquals(
            applicant.getContactDetails().getPostcode(),
        ...
        Assertions.assertEquals(
            applicant.getContactDetails().getMobile(),
            standardApplicant.getMobileNumber()
        );
        Assertions.assertEquals(
            applicant.getContactDetails().getEmail(),
            standardApplicant.getEmailAddress()
        );
    }

    public static void validateOrganisation(Organisation applicant,
                                      StandardApplicant standardApplicant) {
    ...
    public static void validatePerson(Person applicant,
                                NameAddress applicationListEntry) {
        Assertions.assertNull(
            applicationListEntry.getName()
        );

        // assert applicant
        Assertions.assertEquals(
            applicant.getName().getSurname(),
            applicationListEntry.getSurname()
        );
        Assertions.assertEquals(
            applicant.getName().getThirdForename(),
        ...
        Assertions.assertEquals(
            applicant.getName().getFirstForename(),
            applicationListEntry.getForename1()
        );
        Assertions.assertEquals(
            applicant.getName().getSecondForename(),
        ...
        Assertions.assertEquals(
            applicant.getContactDetails().getPostcode(),
            applicationListEntry.getPostcode()
        );
        Assertions.assertEquals(
            applicant.getContactDetails().getAddressLine1(),
