...
/**
 * A useful assertion class for the create/update application entry functionality. This class
 * aims to handle the bulk of the core validation
 *
 * This class takes the request and response of the operation as well as the resulting database entity. From this
 * information it can assert what the database entity should look like as well as the response.
 *
 */
@Component
...
public class ApplicationListEntryAssertion {
     ...
     * @param assertWording The wording we expect to result from the associated code wording template
     * @param expectedWordingFields The fields associated with the wording template that
     ...
     * @param expectedWordingFields The fields associated with the wording template that
     *                              we expect in the response
     * @param existingFeeStatuses When updating these are the fee statuses that already exist prior to the update
     *                            operation taking place. This is used to offset the
     *                            validation of the api response and database records
     */
    @Transactional
    public void validateEntityAndResponseForEntryUpdate(
        ...
        if (entryCreateDto.getApplicant() != null && (entryCreateDto.getApplicant().getPerson() != null
        ...
        if (entryCreateDto.getRespondent() != null && (entryCreateDto.getRespondent().getPerson() != null
        ...
        if (entryCreateDto.getNumberOfRespondents() != null) {
            Assertions.assertEquals(entryCreateDto.getNumberOfRespondents(), applicationListEntry.getNumberOfBulkRespondents().intValue());
        }
        ...
        if (entryCreateDto.getFeeStatuses() != null) {
            for (int i = 0; i < entryCreateDto.getFeeStatuses().size(); i++) {
                if (i >= existingFeeStatuses.size()) {
                    Assertions.assertEquals(
                        entryCreateDto.getFeeStatuses().get(i - existingFeeStatuses.size()).getPaymentReference(),
                        fees.get(i).getAlefsPaymentReference()
                    );
                    Assertions.assertEquals(
                        ApplicationListEntryEntityMapper.toStatus(
                            entryCreateDto.getFeeStatuses().get(i - existingFeeStatuses.size()).getPaymentStatus()),
        ...
        if (entryCreateDto.getFeeStatuses() != null) {
            ...
            Assertions.assertEquals(
                ...
                entryCreateDto.getFeeStatuses().size() + existingFeeStatuses.size()
            );
        }
        ...
        for (int i = 0; i < response.getFeeStatuses().size(); i++) {
            ...
            if (i >= existingFeeStatuses.size()) {
                ...
                Assertions.assertEquals(
                    entryCreateDto.getFeeStatuses().get(i - existingFeeStatuses.size()).getStatusDate(),
