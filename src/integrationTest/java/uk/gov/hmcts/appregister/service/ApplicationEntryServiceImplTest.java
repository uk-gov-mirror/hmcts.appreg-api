...
public class ApplicationEntryServiceImplTest extends BaseIntegration {
    ...
    public void createEntryNoRespondentWithOffsiteFee() {
        ...
        // run the test
        response = unitOfWork.inTransaction(()-> {
        ...
        unitOfWork.inTransaction(()-> {
            ApplicationList applicationList = applicationListRepository.findAll().getFirst();
            List<ApplicationListEntry> entries = applicationListEntryRepository.findByApplicationListId(applicationList.getId());
            ...
            validateEntityAndResponseForEntryCreation(entryCreateDto,
     ...
     */
    private void validateEntityAndResponseForEntryCreation(EntryCreateDto entryCreateDto,
    ApplicationListEntry applicationListEntry, EntryGetDetailDto response,
        ...
        if (entryCreateDto.getStandardApplicantCode() != null) {
           ...
           Assertions.assertEquals(entryCreateDto.getStandardApplicantCode(),
                                   applicationListEntry.getStandardApplicant().getApplicantCode());
        }
        else {
           if (entryCreateDto.getApplicant().getPerson() != null) {
               ApplicantAssertion.validatePerson(entryCreateDto.getApplicant().getPerson(), applicationListEntry.getAnamedaddress());
           } else {
               ApplicantAssertion.validateOrganisation(entryCreateDto.getApplicant().getOrganisation(),
        ...
        if (entryCreateDto.getRespondent() != null) {
            if (entryCreateDto.getRespondent().getPerson() != null) {
                ApplicantAssertion.validatePerson(entryCreateDto.getRespondent().getPerson(), applicationListEntry.getRnameaddress());
            } else {
                ApplicantAssertion.validateOrganisation(entryCreateDto.getRespondent().getOrganisation(), applicationListEntry.getRnameaddress());
            }
            Assertions.assertEquals(entryCreateDto.getRespondent().getDateOfBirth(), applicationListEntry.getRnameaddress().getDateOfBirth());
        }

        // make sure the code of the applicant and respondent are set correctly
        if (entryCreateDto.getApplicant().getPerson()!=null || entryCreateDto.getApplicant().getOrganisation()!=null ) {
            // validate the application code
            Assertions.assertEquals(
                NameAddress.APPLICANT_CODE,
                applicationListEntry.getAnamedaddress().getCode()
            );
        } else if (entryCreateDto.getRespondent().getPerson()!=null || entryCreateDto.getRespondent().getOrganisation()!=null) {
            // validate the application code
            Assertions.assertEquals(
                NameAddress.RESPONDENT_CODE,
        ...
        if (entryCreateDto.getFeeStatuses()!=null && !entryCreateDto.getFeeStatuses().isEmpty()) {
            Assertions.assertEquals(entryCreateDto.getHasOffsiteFee(),
        ...
        Assertions.assertEquals(entryCreateDto.getLodgementDate(), applicationListEntry.getLodgementDate());
        Assertions.assertEquals(assertWording, applicationListEntry.getApplicationListEntryWording());

        // validate the fees are created in the database
        List<AppListEntryFeeStatus> fees = appListEntryFeeStatusRepository
            .findByAppListEntryId(applicationListEntry.getId());
        ...
        if (entryCreateDto.getFeeStatuses()!=null) {
            for (int i = 0; i < entryCreateDto.getFeeStatuses().size(); i++) {
                ...
                Assertions.assertEquals(
                    ApplicationListEntryEntityMapper.toStatus(entryCreateDto.getFeeStatuses().get(i).getPaymentStatus()),
        ...
        List<AppListEntryOfficial> originals = applicationListEntryOfficialRepository
            .findByAppListEntryId(applicationListEntry.getId());
        ...
        if (entryCreateDto.getStandardApplicantCode() != null) {
            Applicant applicant = StandardApplicantMapper.toApplicant(applicationListEntry.getStandardApplicant());
            if (applicant.getPerson() != null) {
                ApplicantAssertion.validatePerson(response.getApplicant().getPerson(),
                                                  applicationListEntry.getStandardApplicant());
            } else {
                ApplicantAssertion.validateOrganisation(response.getApplicant().getOrganisation(), applicationListEntry
                    .getStandardApplicant());
            }
        }
        else {
            if (entryCreateDto.getApplicant().getPerson() != null) {
                ApplicantAssertion.validatePerson(response.getApplicant().getPerson(), applicationListEntry.getAnamedaddress());
            } else {
                ApplicantAssertion.validateOrganisation(response.getApplicant().getOrganisation(), applicationListEntry.getAnamedaddress());
            }
        }
        ...
        if (entryCreateDto.getRespondent() != null) {
            if (entryCreateDto.getRespondent().getPerson() != null) {
                ApplicantAssertion.validatePerson(response.getRespondent().getPerson(),
                                                  applicationListEntry.getRnameaddress());
            } else {
                ApplicantAssertion.validateOrganisation(response.getRespondent().getOrganisation(),
                                                        applicationListEntry.getRnameaddress());
            }
            Assertions.assertEquals(response.getRespondent().getDateOfBirth(), applicationListEntry.getRnameaddress().getDateOfBirth());
        }
        ...
        Assertions.assertEquals(applicationListEntry.getApplicationList().getUuid(), response.getListId());
