...
public class ApplicationCodeRepositoryTest extends BasePostgresIntegrationTest {
    ...
    public void testBasicInsertionUpdate() throws Exception{
        // test save
        ApplicationCode code = persistance.saveAppCode(new ApplicationCodeTestData().someMinimal().build());
        ...
        // test get
        Optional<ApplicationCode> applicationCodeToAssertAgainst = applicationCodeRepository.findById(code.getId());
        ...
        assertEquals(code.getApplicationCode(), applicationCodeToAssertAgainst.get().getApplicationCode());
        assertEquals(code.getCreatedUser(), applicationCodeToAssertAgainst.get().getCreatedUser());
        assertTrue(DateUtil.equalsIgnoreMillis(code.getEndDate(), applicationCodeToAssertAgainst.get().getEndDate()));
        assertTrue(DateUtil.equalsIgnoreMillis(code.getStartDate(), applicationCodeToAssertAgainst.get().getStartDate()));
        assertEquals(code.getBulkRespondentAllowed(), applicationCodeToAssertAgainst.get().getBulkRespondentAllowed());
        ...
        assertNull(code.getDestinationEmail1(), applicationCodeToAssertAgainst.get().getDestinationEmail1());
        assertNull(code.getDestinationEmail2(), applicationCodeToAssertAgainst.get().getDestinationEmail2());
    }
}
